...
INSERT INTO DEPARTMENTS (
    DEPARTMENT_ID, DEPARTMENT_NAME, CITY, STATE) 
    VALUES (
    10, 'Accounting', 'Pisgah', 'NV');

INSERT INTO DEPARTMENTS (
    DEPARTMENT_ID, DEPARTMENT_NAME, CITY, STATE) 
    VALUES (
    11, 'Production', 'Redlands', 'CA');

INSERT INTO DEPARTMENTS (
    DEPARTMENT_ID, DEPARTMENT_NAME, CITY, STATE) 
    VALUES (
    12, 'Engineering', 'Dallas', 'TX');
...
INSERT INTO EMPLOYEES (EMPLOYEE_ID, FIRST_NAME, LAST_NAME, JOB_ID, MANAGER_ID, HIRE_DATE, SALARY, DEPARTMENT_ID)
    VALUES (2723, 'Pasty', 'Gaudet', 96, 19, 
    TO_Date ('04/05/1998 01:11:12 PM', 'MM/DD/YYYY HH:MI:SS AM'),
    87662.97, 10);

INSERT INTO EMPLOYEES (EMPLOYEE_ID, FIRST_NAME, LAST_NAME, JOB_ID, MANAGER_ID, HIRE_DATE, SALARY, DEPARTMENT_ID)
    VALUES (4034, 'Mario', 'Atwell', 2, 9, 
    TO_Date ('12/08/1998 01:11:12 PM', 'MM/DD/YYYY HH:MI:SS AM'),
    109001.61, 11);
...
CREATE OR REPLACE PACKAGE PKG_APP_AUDIT INSERT
...
CREATE OR REPLACE PACKAGE BODY PKG_APP_AUDIT INSERT

    G_TABLE_ID NUMBER := NULL;
    G_COLUMN_SPEC VARCHAR2(4000) := NULL;

    --
    -- THIS FUNCTION RETURNS THE COLUMN_CLAUSE
    -- OF THE SELECT STATEMENT FOR EXECUTE IMMDIATE
    --
    ...
    --
    -- THIS FUNCTION RETURNS THE USING CLAUSE
    -- OF THE EXECUTE IMMDIATE STATEMENT
    --
    ...
    BEGIN
        ...
        IF (SYSDATE <= V_REC.START_DATE OR 
         SYSDATE >= V_REC.END_DATE) OR
          V_REC.AUDIT_FLAG <> 'Y' THEN
         RETURN FALSE;
        END IF;
    ...
    BEGIN
        ...
        IF G_COLUMN_SPEC IS NULL OR
        UPPER(G_COLUMN_SPEC) = 'ALL' THEN
        RETURN P_TMP;
    END IF;
        ...
        LOOP
        V_POS2 := INSTR(P_TMP, '~', V_POS1);
    EXIT WHEN V_POS2 = 0;
    V_STR := SUBSTR(P_TMP, V_POS1, V_POS2 - V_POS1);
...
END;
    ...
    BEGIN
        ...
        VALUES(SEQ_APP_AUDIT_DATA.NEXTVAL, G_TABLE_ID, P_COLUMN_VAL, SYSDATE, USER, 'A');
    END;
    --
...
FUNCTION AUDIT_CHECK RETURNS BOOLEAN

FUNCTION GET_COL_CLAUSE RETURNS VARCHAR2

FUNCTION GET_OLD_CLAUSE RETURNS VARCHAR2

PROCEDURE INSERT_DATA

FUNCTION PARSE_VALUES RETURNS VARCHAR2

------------------------------------------------------

CREATE OR REPLACE TRIGGER TRG_TableName_BUDR
...
BEGIN
    ...
    V_FLAG := PKG_APP_AUDIT.AUDIT_CHECK( 'TableName' );
    IF V_FLAG = TRUE THEN
       V_COL := PKG_APP_AUDIT.GET_COL_CLAUSE( 'TableName' );

       EXECUTE IMMEDIATE 'SELECT' || V_COL || 'FROM DUAL'
        INTO V_TMP
        USING :OLD.ColumnName1, :OLD.ColumnName2, ... , :OLD.ColumnNameN;

       V_TMP := PKG_APP_AUDIT.PARSE_VALUES(V_TMP);
       PKG_APP_AUDIT.INSERT_DATA('TableName', V_TMP);
    END IF;
...
INSERT INTO APP_AUDIT_TABLE(TABLE_ID, TABLE_NAME, TABLE_DESCRIPTION, AUDIT_FLAG,
                    ...
                    VALUES(1, 'EMPLOYEES', 'EMPLOYEE TABLE HOLDING STAFF DATA',
                            'Y', 'Y', 5, 'Y', 3, '[3][6][7]', 2, 
                            TRUNC(SYSDATE), TRUNC(SYSDATE+5),
                            SYSDATE, SYSDATE, 'DBSEC', 'A');

INSERT INTO APP_AUDIT_TABLE(TABLE_ID, TABLE_NAME, TABLE_DESCRIPTION, AUDIT_FLAG,
                            ARCHIVE_FLAG, ARCHIVE_COUNT, PURGE_FLAG, PURGE_COUNT, 
                            COLUMN_NAMES, COLUMNS_COUNT, START_DATE, END_DATE, 
                            CTL_INS_DTTM, CTL_UPD_DTTM, CTL_UPD_USER, CTL_REC_STAT)
                    VALUES(2, 'DEPARTMENTS', 'DEPARTMENT TABLE',
                            'Y', 'N', 0, 'N', 0, 'ALL', 3, 
                            TRUNC(SYSDATE), TRUNC(SYSDATE+5),
                            SYSDATE, SYSDATE, 'DBSEC', 'A');
...
SELECT T.TABLE_NAME,
       D.AUDIT_DATA,
       TO_CHAR(D.CTL_INS_DTTM, 'DD-MM-YYYY HH24:MM:SS') DATE_CREATED,
       D.CTL_UPD_USER USERNAME
    FROM APP_AUDIT_TABLE T,
         APP_AUDIT_DATA D
